// background.js - Suno Watcher Beta
...
const BATCH_KEY = 'sunoSimpleDlSettings';
async function currentBatch() {
    try { const r = await api.storage.local.get(BATCH_KEY); return Math.max(1, Math.min(5, parseInt(r && r[BATCH_KEY] && r[BATCH_KEY].batchSize, 10) || 1)); } catch (e) { return 1; }
}
function buildMenus(current) {
    try {
        api.contextMenus.removeAll(() => {
            api.contextMenus.create({ id: 'batch_parent', title: 'Parallel downloads', contexts: ['action'] });
            for (let i = 1; i <= 5; i++) {
                api.contextMenus.create({ id: 'batch_' + i, parentId: 'batch_parent', type: 'radio', checked: i === current, title: String(i), contexts: ['action'] });
            }
        });
    } catch (e) {}
}
api.runtime.onInstalled.addListener(async () => { buildMenus(await currentBatch()); });
(async () => { buildMenus(await currentBatch()); })();
if (api.contextMenus && api.contextMenus.onClicked) {
    api.contextMenus.onClicked.addListener((info) => {
        const id = String(info.menuItemId || '');
        if (id.startsWith('batch_') && id !== 'batch_parent') {
            const n = parseInt(id.split('_')[1], 10);
            if (n >= 1 && n <= 5) api.storage.local.set({ [BATCH_KEY]: { batchSize: n } });
        }
    });
}
...
api.runtime.onMessage.addListener((msg, _sender, sendResponse) => {
    ...
    if (msg.action === 'ensure_watcher') {
        (async () => {
            const tab = await getSunoTab();
            if (!tab || !tab.id) { sendResponse({ ok: false }); return; }
            try { await api.scripting.executeScript({ target: { tabId: tab.id }, files: ['watcher.js'] }); sendResponse({ ok: true }); }
